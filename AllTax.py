...
class AllTYear():
    def __init__(self):
        SOFT_NAME = "Quaterly Report © version 1"
        ...
        layout = [[sg.T("")],[sg.Image(filename=fname, size=(300, 120))], \
                 [sg.Text("Choose Month 1 AllTax Report(.xlsx): "), sg.Input(), sg.FileBrowse(key="-IN1-")], \
                  [sg.Text("Choose Month 2 AllTax Report(.xlsx):  "), sg.Input(), sg.FileBrowse(key="-IN2-")], \
                  [sg.Text("Choose Month 3 AllTax Report(.xlsx): "), sg.Input(), sg.FileBrowse(key="-IN3-")], \
                    [sg.Text("Choose Final Quarterly Report Destination: "), sg.Input(key="-IN4-" ,change_submits=True), sg.FolderBrowse(key="-IN4-")], \
                  [sg.Button('Aggregate Reports'), sg.Exit()]]
        ...
        while True:
            ...
            if event == sg.WIN_CLOSED or event=="Exit":
                ...
                break
            elif event == "Aggregate Reports":
                filpA = values["-IN1-"]
                filpB = values["-IN2-"]
                filpC = values["-IN3-"]
                save_path = values["-IN4-"]
                try:
                    assert filpA.split('.')[-1] == 'xlsx'
                    try:
                        assert filpC.split('.')[-1] == 'xlsx'
                        try:
                            assert filpB.split('.')[-1] == 'xlsx'
                            try:
                            ...
                            sg.popup('Entered path {} is not .xlsx, Please try again!'.format(filpB))
                    except:
                        sg.popup('Entered path {} is not .xlsx, Please try again!'.format(filpC))

                except:
                    sg.popup('Entered path {} is not .xlsx, Please try again!'.format(filpA))
        window.close()

        writer = pd.ExcelWriter(os.path.join(save_path, 'Quarterly_Report_{}_{}_{}.xlsx'.format(now.month, now.day, now.year)),engine='xlsxwriter')   
        ...
        def get_dataframe(file_name):
            ...
            df1 = pd.read_excel(xls, 'Overall Sales-State')
            return df1
        ...
        df = pd.DataFrame()
        for paths in [filpA, filpB, filpC]:
            df = df.append((get_dataframe(paths)))

        n_by_state = df.groupby("Place of Supply").sum().round(decimals=2)
        print("================================ Quarterly Sales State Breakdown ============================================")

        print(n_by_state)
        save_daraframe(n_by_state,'Quarterly-Sales')
        writer.save()
        print_separator()
...
class AllTMonth():
    def __init__(self):
        SOFT_NAME = "AllTax Monthly © version 6"
        ...
        Fk_REQUIRED_FIELDS = ['Taxable Value (Final Invoice Amount -Taxes)', \
                          ...
                          'TCS IGST Amount','TCS SGST Amount','TCS CGST Amount','Customer\'s Billing State']
...
##window.close()
    ...
    def get_flipkart(fk_filename, Fk_REQUIRED_FIELDS):
        ...
        df.rename(columns={"Customer\'s Billing State":"Place of Supply", "SGST Amount (Or UTGST as applicable)": "SGST Amount", "Taxable Value (Final Invoice Amount -Taxes)": "Taxable Value"}, inplace=True)
